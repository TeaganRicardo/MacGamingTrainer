...
ConsumableData = {
  RoomMoneyDrop = {}, RoomMoneyBigDrop = {}, RoomMoneyTripleDrop = {},
...
GiveLoot = function(args)
  if args.ForceLootName=="SpellDrop" then
    loadedPackages.SpellPackage=true
    if PregenerateSpells then PregenerateSpells() end
  end
  nextObjectId=nextObjectId+1; spawnedLoot=args.ForceLootName; return { ObjectId=nextObjectId, Name=args.ForceLootName, CanReceiveGift=args.ForceLootName=="SpellDrop" }
end
...
assert(table.concat(resourceIds,',')=='GiftPoints,MetaCurrency,OreFSilver','ResourceDisplayOrderData was not preserved exactly')
assert(state.resources[1].sectionTitle=='赠礼资源' and state.resources[3].sectionTitle=='矿物','resource section metadata missing')
...
assert(spawnRoomRewardCalls==1 and lastSpawnRoomRewardArgs.RewardOverride=='SpellDrop' and lastSpawnRoomRewardArgs.LootName==nil,'Selene spawn bypassed native room reward pipeline')
