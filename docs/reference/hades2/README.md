...
- `weapons.csv` contains 1,136 rows across player, enemy, trap, NPC, and helper weapons. Its three `stub` rows are all source-valid: `WeaponStaffBall2` is an empty compatibility/objective marker explicitly commented as "Only used for misnamed objective" and referenced only by objective data; `WeaponSuitDash` is a real `WeaponSuit`-derived dash weapon referenced by multiple boon/aspect property changes; `RubbleFallLarge` is a real `RubbleFall`-derived trap weapon included in `WeaponSets.RubbleWeapons` and used by obstacle/projectile logic.
