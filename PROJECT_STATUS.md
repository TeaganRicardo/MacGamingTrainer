...
`DoubleFamiliarTrait` is now real-game confirmed through both exact direct acquisition and Circe native choice. Its effect applies directly to the active Familiar rather than surfacing as a separate standalone trait icon, so the existing `direct` ledger classification is confirmed. Two acceptance items remain before treating revision 46 as fully closed: representative Charon-Well behavior through the `pickup` / `卡戎之井` section, and one same-PID main-menu -> run lifecycle round trip on this exact artifact.
